...
async function main() {
  ...
  console.log('--- Importing 2026 Receipts for Immeuble 4 ---');
  ...
  for (let i = start; i < rows.length; i++) {
    ...
    if (inData && row && typeof row[0] === 'number') {
      ...
      const unitId = lotToUnitId.get(lotNum);

      if (!unitId) {
        console.warn(`Unit not found for lot ${lotNum}`);
        continue;
      }

      // Check months (Jan to Dec are columns 2 to 13, Garage is 14)
      for (let m = 2; m <= 14; m++) {
        ...
        if (amount > 0) {
          ...
          if (m <= 13) {
            // Jan is m=2 -> month=0
            date = new Date(Date.UTC(2026, m - 2, 1));
          } else {
            // Garage (m=14) -> let's put it at 2026-12-31 or something similar
  ...
  console.log(`Total Amount imported: ${totalAmount} DH`);
}
