...
async function run() {
  ...
  console.log('--- RÉCONCILIATION GLOBALE (IMM 2 À 6) ---');
  ...
  for (const b of BUILDINGS) {
    ...
    for (let i = 0; i < excelRows.length; i++) {
      if (excelRows[i] && excelRows[i].includes('Immeuble') && excelRows[i].includes(parseInt(b.name.split(' ')[1]))) {
        startRow = i;
        break;
      }
    }
        ...
        for (let m = 2; m <= 14; m++) {
          ...
          if (val) {
            if (typeof val === 'number') {
              ...
              // Heuristic: for standard months without amount, count 140 or 130? 
              // We'll skip count for "PAYE" text as we want money sum primarily.
            }
          }
        }
      }
    ...
    const dbOther = await prisma.otherReceipt.findMany({
      where: { OR: [{ description: { contains: b.short, mode: 'insensitive' } }, { note: { contains: b.short, mode: 'insensitive' } }] }
    });
    ...
    console.log(`  Excel Sum: ${excelTotal} MAD`);
    console.log(`  DB Total:  ${dbTotal} MAD (Std: ${sumStandard}, Other: ${sumOther})`);
    
    if (Math.abs(excelTotal - dbTotal) < 1) {
      console.log(`  ✅ SYNC OK`);
    } else {
      console.log(`  ❌ DIFF: ${excelTotal - dbTotal} MAD`);
    }
  }
