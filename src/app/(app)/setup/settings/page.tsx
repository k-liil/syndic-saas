...
export default function SettingsPage() {
  ...
  const primaryAction = useMemo(() => {
    ...
    if (tab === "contributions") {
      return {
        label: savingSettings ? "Enregistrement..." : "Enregistrer les rÃ©glages",
  ...
  async function saveContributionSettings() {
    ...
    try {
      ...
      await loadSettings();
      showStatus("success", "Calcul des cotisations mis Ã  jour.");
    } catch {
  ...
  async function deleteGroup(groupId: string) {
    ...
    await loadContributions();
    showStatus("success", "Groupe supprimÃ©.");
  }

  async function addPeriod() {
    if (!periodStart || !periodAmount) return;
    setSavingSettings(true);
    showStatus("saving", "Ajout de la pÃ©riode...");
    try {
      ...
      setPeriodAmount("");
      await loadContributions();
      showStatus("success", "PÃ©riode ajoutÃ©e.");
    } catch (e: any) {
  ...
  async function deletePeriod(periodId: string) {
    if (!confirm("Supprimer cette pÃ©riode ?")) return;
    showStatus("saving", "Suppression...");
    await fetch(apiUrl(`/api/contribution-periods?id=${periodId}`), { method: "DELETE" });
    await loadContributions();
    showStatus("success", "PÃ©riode supprimÃ©e.");
  }
  ...
  async function runSimulation() {
    ...
    try {
      ...
      showStatus("success", "Simulation terminÃ©e.");
    } catch {
  ...
  async function confirmDelete() {
    ...
    if (!res.ok) {
      ...
      await loadBanks();
      showStatus("success", "Ã‰lÃ©ment supprimÃ©.");
    }
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        title={bank.isActive ? "DÃ©sactiver" : "RÃ©activer"}
                      >
                        <span
                          className={`inline-block h-4 w-4 transform rounded-md bg-white transition ${
