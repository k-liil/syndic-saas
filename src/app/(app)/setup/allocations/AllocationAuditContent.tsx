...
import {
  getUnitAuditData,
  deleteReceiptAndReallocate,
  forceRecalculateUnit,
} from "./actions";
...
export function AllocationAuditContent() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setSearching(true);
      logger.addLog(`Recherche de lot pour "${search}"...`, "info");
      try {
        ...
        logger.addLog(`Erreur de recherche: ${err}`, "error");
      } finally {
        setSearching(false);
      }
    }, 400);
  ...
  const handleSelectUnit = (unit: any) => {
    logger.addLog(
      `Lot ${unit.lotNumber} sélectionné. Chargement de l'audit...`,
      "info",
    );
  ...
  const handleDeleteReceipt = async (receiptId: string) => {
    if (
      !confirm(
        "Voulez-vous vraiment supprimer cet encaissement ? Toutes ses allocations seront perdues et le lot sera recalculé.",
      )
    )
      return;
    setProcessing(true);
    logger.addLog(
      `Demande de suppression reçue pour l'encaissement ${receiptId}...`,
      "info",
    );
    try {
      const res = await deleteReceiptAndReallocate(receiptId);
      if (res.logs && res.logs.length > 0) {
        logger.addLogs(res.logs, res.ok ? "info" : "error");
      }
      if (res.ok) {
        logger.addLog(
          `Suppression réussie. Actualisation de la vue...`,
          "success",
        );
        fetchAudit(selectedUnit.id);
      } else {
        logger.addLog(`Erreur lors de la suppression: ${res.error}`, "error");
        alert("Erreur: " + res.error);
      }
    } catch (err) {
      logger.addLog(`Exception inattendue: ${err}`, "error");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleForceRecalculate = async () => {
    ...
    setProcessing(true);
    logger.addLog(
      `Lancement de Force Recalculate pour l'unité ${selectedUnit.id}...`,
      "warning",
    );
    try {
      ...
      if (res.ok) {
        logger.addLog(
          `Recalcul terminé avec succès. Actualisation...`,
          "success",
        );
        fetchAudit(selectedUnit.id);
      } else {
        logger.addLog(`Échec du recalcul: ${res.error}`, "error");
        alert("Erreur: " + res.error);
      }
    } catch (err) {
      logger.addLog(`Exception pendant le recalcul: ${err}`, "error");
    } finally {
      setProcessing(false);
    }
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
              <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-800">
                  Audit des Reçus & Allocations
                </h3>
              </div>
              <div className="p-4 space-y-4 overflow-auto max-h-[600px] bg-slate-50/30">
                {auditData.receipts.map((r: any) => {
                  const allocations = auditData.allocations.filter(
                    (a: any) => a.receiptId === r.id,
                  );
                  return (
                    <div
                      key={r.id}
                      className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:border-indigo-200 transition-all group"
                    >
                ...
                {auditData.receipts.length === 0 && (
                  <div className="text-center py-20 flex flex-col items-center">
                    ...
                    <p className="text-sm text-slate-400 italic">
                      Aucun reçu de contribution trouvé pour ce lot.
                    </p>
                  </div>
                )}
              </div>
      ...
      {!selectedUnit && (
        <div className="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/50 py-24 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Choisissez un lot pour démarrer l'audit.
          </p>
        </div>
      )}
